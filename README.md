...
```
guppy_basecaller --input minKNOW_input/ --save_path basecalled_fastqs/ -c dna_r9.4.1_450bps_fast.cfg --recursive
```

Basecalling and filter reads by quality score (here, set to 10):

```
guppy_basecaller --input minKNOW_input/ --save_path basecalled_fastqs/ -c dna_r9.4.1_450bps_fast.cfg --recursive --min_qscore 10
```
...
B5 | Demultiplexing of the sequencing data with [minibar](https://github.com/calacademy-research/minibar) or [qcat](https://github.com/nanoporetech/qcat)

- minibar:

```
python minibar.py indexfile.txt sequencing_reads.fastq -T -F -e 3 -E 11
```
...
```
qcat -b demultiplexed_files -f sequencing_reads.fastq -trim
```
...
- intended target length (`--m`) = 400 base pairs
- maximum deviation from target length (`--s`) = 50 base pairs
